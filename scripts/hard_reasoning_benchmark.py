...
Does the ensemble beat the best single model on REASONING tasks?
Not factual recall — multi-step math, logic, code debugging.

Conditions:
1. Best single model (per-prompt oracle)
2. Uniform ensemble (majority vote)
3. Consensus-weighted (similarity agreement)
4. Judge synthesis (7B synthesizes best answer)

Key difference from competence_benchmark.py:
- Harder prompts (GSM8K multi-step math, logic puzzles, code debugging)
- Measures reasoning steps, not just final answer
- Tests synthesis quality explicitly
"""
import json
import re
import sys
...
MATH_REASONING = [
    ("A train travels at 60 mph for 2 hours, then 40 mph for 3 hours. What is the average speed?",
     "48",  # total distance = 120+120=240, total time = 5, avg = 48
     ["Step 1: Distance at 60mph = 60*2 = 120 miles",
      "Step 2: Distance at 40mph = 40*3 = 120 miles",
      "Step 3: Total distance = 240 miles, total time = 5 hours",
      "Step 4: Average speed = 240/5 = 48 mph"]),

    ("If a rectangle has length 12 and width 5, and a triangle has base 12 and height 5, what is the ratio of their areas?",
     "2",  # rect area = 60, tri area = 30, ratio = 2
     ["Step 1: Rectangle area = 12*5 = 60",
      "Step 2: Triangle area = 0.5*12*5 = 30",
      "Step 3: Ratio = 60/30 = 2"]),

    ("You buy 3 shirts at $15 each and 2 pairs of pants at $25 each. You get a 20% discount. How much do you pay?",
     "76",  # 3*15=45, 2*25=50, total=95, 20% off = 95*0.8 = 76
     ["Step 1: Shirts cost = 3*15 = $45",
      "Step 2: Pants cost = 2*25 = $50",
      "Step 3: Total = $95",
      "Step 4: After 20% discount = 95*0.8 = $76"]),

    ("A pool is filled by pipe A in 6 hours and pipe B in 4 hours. How long to fill the pool with both open?",
     "2.4",  # 1/6 + 1/4 = 5/12, time = 12/5 = 2.4
     ["Step 1: Pipe A rate = 1/6 pool per hour",
      "Step 2: Pipe B rate = 1/4 pool per hour",
      "Step 3: Combined rate = 1/6 + 1/4 = 5/12",
      "Step 4: Time = 12/5 = 2.4 hours"]),

    ("If you invest $1000 at 5% annual compound interest, how much do you have after 3 years?",
     "1157.63",  # 1000 * 1.05^3 = 1157.625
     ["Step 1: Year 1 = 1000 * 1.05 = 1050",
      "Step 2: Year 2 = 1050 * 1.05 = 1102.50",
      "Step 3: Year 3 = 1102.50 * 1.05 = 1157.625"]),

    ("A car uses 8 liters per 100km. Gas costs $1.50 per liter. How much does a 240km trip cost?",
     "28.80",  # 240/100*8 = 19.2 liters, 19.2*1.5 = 28.80
     ["Step 1: Fuel needed = 240/100 * 8 = 19.2 liters",
      "Step 2: Cost = 19.2 * 1.50 = $28.80"]),

    ("Three workers can build a wall in 4 days. How many workers are needed to build it in 1 day?",
     "12",  # 3 workers * 4 days = 12 worker-days, so 12 workers for 1 day
     ["Step 1: Work = 3 workers * 4 days = 12 worker-days",
      "Step 2: For 1 day = 12 workers needed"]),

    ("A bakery sells cupcakes for $2.50 each. They sell 40 per day. How many days to earn $500?",
     "5",  # 40*2.50 = 100/day, 500/100 = 5
     ["Step 1: Daily revenue = 40 * 2.50 = $100",
      "Step 2: Days needed = 500/100 = 5"]),

    ("If 5 machines make 5 widgets in 5 minutes, how long for 100 machines to make 100 widgets?",
     "5",  # 1 machine makes 1 widget in 5 minutes
     ["Step 1: 5 machines = 5 widgets in 5 min",
      "Step 2: 1 machine = 1 widget in 5 min",
      "Step 3: 100 machines = 100 widgets in 5 min"]),

    ("You have a 3-gallon jug and a 5-gallon jug. How do you measure exactly 4 gallons?",
     "4",  # Fill 5, pour to 3, empty 3, pour 2 from 5 to 3, fill 5, pour 1 to fill 3 = 4 in 5-gal
     ["Step 1: Fill 5-gallon jug",
      "Step 2: Pour from 5 into 3 until full (3 gal in 3-jug, 2 gal in 5-jug)",
      "Step 3: Empty 3-gallon jug",
      "Step 4: Pour 2 gallons from 5 into 3",
      "Step 5: Fill 5-gallon jug",
      "Step 6: Pour from 5 into 3 until full (needs 1 more gallon)",
      "Step 7: 5-gallon jug now has exactly 4 gallons"]),
]
...
LOGIC_PUZZLES = [
    ("If all roses are flowers, and some flowers fade quickly, can we conclude that some roses fade quickly?",
     "No",
     ["This is an invalid syllogism. 'Some flowers fade quickly' doesn't mean those flowers are roses."]),

    ("A farmer has 17 sheep. All but 9 die. How many are left?",
     "9",
     ["'All but 9 die' means 9 survive. The answer is 9."]),
    ...
    ("Two coins add up to 30 cents. One of them is not a nickel. What are the two coins?",
     "Quarter and nickel",
     ["One coin is NOT a nickel (the quarter), the other IS a nickel."]),

    ("A doctor gives you 3 pills and says to take one every 30 minutes. How long do they last?",
     "60 minutes",
     ["Take pill 1 at t=0, pill 2 at t=30, pill 3 at t=60. Lasts 60 minutes."]),

    ("If you have a bowl with six apples and you take away four, how many do YOU have?",
     "4",
     ["You took 4, so YOU have 4. The bowl has 2 left."]),
...
def extract_number(text: str) -> Optional[float]:
    ...
    # Look for last number in output
...
def extract_answer(output: str) -> str:
    """Extract the core answer from model output (general)."""
    if not output.strip():
        return ""
    output = output.strip()
...
def run_hard_reasoning_benchmark(
    ...
    """Run the hard reasoning benchmark."""
    results = []

    all_prompts = []

    # Math reasoning
    for prompt, ref, steps in MATH_REASONING:
        all_prompts.append({
            "prompt": prompt,
            "reference": ref,
            "category": "math",
    ...
    # Logic puzzles
    for prompt, ref, steps in LOGIC_PUZZLES:
        all_prompts.append({
            "prompt": prompt,
            "reference": ref,
            "category": "logic",
    ...
    # Code debugging
    for prompt, ref, steps in CODE_DEBUGGING:
        all_prompts.append({
            "prompt": prompt,
            "reference": ref,
            "category": "code",
    ...
    print(f"\nRunning hard reasoning benchmark ({len(all_prompts)} prompts)...")
    ...
    for idx, item in enumerate(all_prompts):
        ...
        for mid in model_ids:
            wrapper = models[mid]
            wrapper.load_to_gpu()
            output = wrapper.generate([prompt], max_new_tokens=256)[0]
        ...
        for mid, out in all_outputs.items():
            if is_numeric:
                accuracy[mid] = check_numeric_match(extract_number(out), ref)
            else:
                accuracy[mid] = check_text_match(out, ref)
...
def compute_metrics(results: List[Dict]) -> Dict:
    ...
    for cat in ["math", "logic", "code"]:
        ...
        for mid in all_model_ids:
            correct = sum(1 for r in cat_results if r["accuracy"].get(mid, False))
            model_acc[mid] = round(correct / len(cat_results), 4) if cat_results else 0
    ...
    model_accuracy = {}
    for mid in all_model_ids:
        correct = sum(1 for r in results if r["accuracy"].get(mid, False))
        model_accuracy[mid] = round(correct / len(results), 4) if results else 0
...
def main():
    print("=" * 80)
    print("HARD REASONING BENCHMARK")
    print("Does the ensemble beat the best single model on REASONING tasks?")
    print("=" * 80)
    ...
    results = run_hard_reasoning_benchmark(models, model_ids, judge_model, judge_tokenizer)
    ...
    for mid, acc in overall["model_accuracy"].items():
    ...
    for cat, cat_data in metrics["by_category"].items():
        ...
        for mid, acc in cat_data["model_accuracy"].items():
            print(f"      {mid:15s}: {acc:.1%}")
    ...
    if vs["ensemble_win_rate"] > 0.6:
        print("VERDICT: PASS — Ensemble beats best single model >60% on reasoning")
        print("The ensemble adds value on hard tasks. Proceed to Phase 2.")
    elif vs["ensemble_win_rate"] > 0.5:
        print("VERDICT: MARGINAL — Ensemble beats best single model 50-60% on reasoning")
        print("Consider upgrading workers before Phase 2.")
    elif vs["ensemble_win_rate"] > 0.4:
        print("VERDICT: NEUTRAL — Ensemble roughly equal on reasoning")
        print("Test with larger/more diverse models before concluding.")
    else:
        print("VERDICT: WEAK — Ensemble does NOT beat best single on reasoning")
        print("Architecture may need redesign.")
    print("=" * 80)

    # Save results
    output_dir = Path("results/hard_reasoning_benchmark")
